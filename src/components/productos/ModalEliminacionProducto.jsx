...
const ModalEliminacionProducto = ({
  ...
  const handleEliminar = async () => {
    if (!producto) return;
    setCargando(true);
    await onConfirmar(producto.id_producto);
    setCargando(false);
  };
  ...
  return (
    <Modal show={mostrar} onHide={onHide} centered backdrop="static" size="sm">
      <Modal.Body className="text-center p-4">
        ...
        <h4 className="fw-bold text-dark mb-2">¿Eliminar producto?</h4>
        <p className="text-muted small mb-4">
          Estás a punto de eliminar <strong>"{producto?.nombre_producto}"</strong>. Esta acción no se puede deshacer.
        </p>
